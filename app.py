...
st.markdown("""
    <style>
        ...
        /* Estilos del Calendario Apple Style */
        .cal-container { background: #0F172A; border-radius: 12px; padding: 15px; border: 1px solid #334155; color: white; }
...
if st.sidebar.button("🔄 Refrescar Datos de API"): st.cache_data.clear()
...
def obtener_calendario_equipo(id_equipo):
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get("response"): fixtures = data.get("response")
...
if live_fixtures:
    for match in live_fixtures:
        records_live.append({
            "Liga": match['league']['name'], "Logo_Liga": match['league']['logo'],
            "Local": match['teams']['home']['name'], "Logo_L": match['teams']['home']['logo'],
            "Goles L": match['goals']['home'], "Visita": match['teams']['away']['name'],
            "Logo_V": match['teams']['away']['logo'], "Goles V": match['goals']['away'],
...
with tab1:
    ...
    for f in historial_raw:
        if 'fixture' in f:
            records_historial.append({
                ...
                "Competencia": f['league']['name'], "Local": f['teams']['home']['name'],
                "Logo_L": f['teams']['home']['logo'], "Goles Local": f['goals']['home'],
                "Goles Visita": f['goals']['away'], "Visita": f['teams']['away']['name'],
                "Logo_V": f['teams']['away']['logo'], "Estado": f['fixture']['status']['short']
            })
    ...
    df_finalizados = df_historial[df_historial['Estado'].isin(['FT', 'AET', 'PEN'])] if not df_historial.empty else pd.DataFrame()
    ...
    with k1: st.markdown(f"<div class='premium-card' style='display:flex; align-items:center; gap:15px; border-left: 4px solid #3B82F6; padding: 15px;'><img src='{logo_activo}' width='50'><div><h3 style='margin:0; font-size:1.2rem;'>{nombre_activo}</h3><small style='color:#94A3B8;'>{pais_activo}</small></div></div>", unsafe_allow_html=True)
    with k2: st.markdown(f"<div class='premium-card' style='padding: 15px;'><small style='color:#94A3B8;'>EFECTIVIDAD</small><h2 style='margin:0; color:#10B981 !important;'>{round((victorias / partidos_jugados) * 100, 1) if partidos_jugados > 0 else 0}%</h2></div>", unsafe_allow_html=True)
    with k3: st.markdown(f"<div class='premium-card' style='padding: 15px;'><small style='color:#94A3B8;'>GOLES / PARTIDO</small><h2 style='margin:0; color:#F59E0B !important;'>{round(goles_favor / partidos_jugados, 1) if partidos_jugados > 0 else 0}</h2></div>", unsafe_allow_html=True)
    ...
    with col_izq:
        ...
        if not df_finalizados.empty:
            for _, row in df_finalizados.head(5).iterrows():
                es_local = row['Local'] == nombre_activo
                g_p = int(row['Goles Local']) if es_local else int(row['Goles Visita'])
                g_r = int(row['Goles Visita']) if es_local else int(row['Goles Local'])
                color_borde = "#10B981" if g_p > g_r else ("#64748B" if g_p == g_r else "#EF4444")
                st.markdown(f"<div style='background: #0F172A; padding: 10px; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid {color_borde};'><div style='display:flex; justify-content:space-between;'><span>{row['Local']}</span><b>{int(row['Goles Local'])} - {int(row['Goles Visita'])}</b><span>{row['Visita']}</span></div></div>", unsafe_allow_html=True)
        else: st.info("No hay resultados recientes.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if plantilla: st.dataframe(pd.DataFrame([{"Número": p.get("number"), "Nombre": p.get("name"), "Edad": p.get("age"), "Posición": p.get("position")} for p in plantilla]), hide_index=True, use_container_width=True)
    else: st.warning("No se encontró información de la plantilla.")
    st.markdown("</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    if df_live.empty: st.info("No hay partidos en vivo.")
        ...
        for _, row in df_live.iterrows():
            st.markdown(f"<div class='live-card'><div style='display: flex; justify-content: space-between; align-items: center;'><div style='width: 35%; display:flex; align-items:center; gap:15px;'><img src='{row['Logo_L']}' width='40'> <span class='live-team-name'>{row['Local']}</span></div><div style='width: 30%; text-align: center;'><span class='live-score'>{row['Goles L']} - {row['Goles V']}</span><br><div style='margin-top:10px;'><span class='pulse-minute'>⏱️ {row['Minuto']}'</span></div></div><div style='width: 35%; display:flex; align-items:center; justify-content:flex-end; gap:15px;'><span class='live-team-name'>{row['Visita']}</span> <img src='{row['Logo_V']}' width='40'></div></div></div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
with tab3:
    ...
    with c1: 
        ...
        st.bar_chart(pd.Series([12, 8, 5, 4, 3], index=['LaLiga', 'Premier', 'Serie A', 'Bundes', 'Ligue 1']), use_container_width=True, color="#3B82F6")
        st.markdown("</div>", unsafe_allow_html=True)
    with c2: 
        st.markdown("<div class='premium-card'><div class='section-title' style='font-size: 1.1rem;'>⚽ Goles Totales</div>", unsafe_allow_html=True)
        st.area_chart(pd.Series([34, 22, 15, 12, 9], index=['LaLiga', 'Premier', 'Serie A', 'Bundes', 'Ligue 1']), use_container_width=True, color="#EF4444")
        st.markdown("</div>", unsafe_allow_html=True)

with tab4:
    st.markdown("<div class='premium-card'><div class='section-title'>🤖 Scout IA</div>", unsafe_allow_html=True)
    pregunta = st.chat_input(f"Pregunta sobre {nombre_activo}...")
    if pregunta: st.chat_message("assistant").write("Análisis táctico en curso...")
    st.markdown("</div>", unsafe_allow_html=True)
