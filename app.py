...
with tab3:
    ...
    with col_der:
        ...
        import random # <--- Asegúrate de importar random al inicio de tu script

# =========================================================================
# CONFIGURACIÓN DE LA API Y RESPALDO
# =========================================================================
# Para obtener métricas 100% reales, regístrate gratis en dashboard.api-football.com 
# y coloca tu nueva llave aquí:
API_KEY = "acb867b68f5987d9c226e48c12c090e3" 
HEADERS = {
    'x-apisports-key': API_KEY,
    'x-rapidapi-host': 'v3.football.api-sports.io'
...
def generar_respaldo_dinamico(nombre_equipo, pais_equipo):
    ...
    if "mexico" in pais_normalizado or "méxico" in pais_normalizado:
        competencia = "Liga MX - Apertura 2026"
        rivales = ["Chivas Guadalajara", "Cruz Azul", "Pumas UNAM", "Tigres UANL", "CF Monterrey", "Toluca"]
    elif "spain" in pais_normalizado or "españa" in pais_normalizado:
        competencia = "La Liga"
        rivales = ["Real Madrid", "Barcelona", "Atlético de Madrid", "Sevilla FC", "Real Betis", "Athletic Club"]
    elif "england" in pais_normalizado or "inglaterra" in pais_normalizado:
        competencia = "Premier League"
        rivales = ["Manchester City", "Arsenal", "Liverpool", "Manchester United", "Chelsea", "Tottenham"]
    else:
        competencia = "Amistoso Internacional"
        rivales = ["Real Madrid", "Paris Saint-Germain", "Bayern Munich", "Manchester City", "Barcelona", "Juventus"]

    # Atamos la semilla de aleatoriedad al nombre del equipo. 
    # Así, las métricas cambiarán entre equipos, pero si buscas al mismo equipo dos veces, sus datos tendrán sentido.
    random.seed(nombre_equipo)
    
    partidos = []
    
    # 1. Generar 4 partidos FINALIZADOS con goles dinámicos
    for i in range(4):
        goles_local = random.randint(0, 4)
        goles_visita = random.randint(0, 3)
        
        # Alternar quién es local y quién es visita
        if i % 2 == 0:
            local, visita = rivales[i], nombre_equipo
        else:
            local, visita = nombre_equipo, rivales[i]
            
        partidos.append({
            "Fecha": f"2026-07-{10 + (i * 4):02d} 18:00", 
            "Competencia": competencia, 
            "Local": local, "Logo Local": logo_generico, 
            "Goles Local": goles_local, "Goles Visita": goles_visita, 
            "Visita": visita, "Logo Visita": logo_generico, "Estado": "FT"
        })
        
    # 2. Generar 2 partidos FUTUROS (sin goles)
    for i in range(4, 6):
        if i % 2 == 0:
            local, visita = rivales[i], nombre_equipo
        else:
        ...
        partidos.append({
            "Fecha": f"2026-08-{5 + (i * 3):02d} 20:00", 
            "Competencia": competencia, 
            "Local": local, "Logo Local": logo_generico, 
            "Goles Local": None, "Goles Visita": None, 
            "Visita": visita, "Logo Visita": logo_generico, "Estado": "NS"
        })
...
def obtener_calendario_equipo(id_equipo, nombre_equipo, pais_equipo):
    ...
    try:
        ...
        if response.status_code == 200:
            res_json = response.json()
            # Validamos si la API de verdad devolvió algo o si nos rechazó por límite
            if res_json.get("response") and len(res_json.get("response")) > 0:
                return res_json.get("response"), "api_directa"
    except Exception:
        pass
    # Si falla o la API Key está caducada, llamamos a la función de respaldo dinámica que acabamos de crear
    return generar_respaldo_dinamico(nombre_equipo, pais_equipo), "local_respaldo"
